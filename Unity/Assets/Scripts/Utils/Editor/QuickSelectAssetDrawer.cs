...
public class QuickSelectAssetDrawer : PropertyDrawer {
	...
	private QuickSelectAssetAttribute Target { get { return m_Target ?? (m_Target = (QuickSelectAssetAttribute) attribute); } }
	...
	private void FindAssets(out GUIContent[] labels, out string[] assetPaths) {
		...
		if (!string.IsNullOrEmpty(Target.Filter)) { assetFilter = Target.Filter; }
		if (!assetFilter.Contains("t:")) { assetFilter += " t:" + fieldInfo.FieldType.Name; }
	...
	public override float GetPropertyHeight(SerializedProperty property, GUIContent label) {
		if (property.objectReferenceValue) { return base.GetPropertyHeight(property, label); }
