...
namespace Deviant.Utils {
	...
	public class Singleton<T> : MonoBehaviour where T : MonoBehaviour {
		...
		public static bool IsValid {
			get { return Application.isPlaying && !AppTracker.IsQuitting; }
		}
				...
				lock (s_Lock) {
					if (s_Instance == null) {
						...
						if (s_Instance == null) {
							...
							//Debug.Log("[Singleton] An instance of " + typeof(T) + " is needed in the scene, so '" + s_Instance + "' was created with DontDestroyOnLoad.");
						}
						else { Debug.Log("[Singleton] Using instance already created: " + s_Instance.name); }
					}
